 ...
 *
 * Description: This file contains the nrf based driver for the TMP116 using
 * TWI or more notably called I2C
 * 
 * Keith Wakeham
 *
...
//stuff to port this to nrf52 series
#include "nrf_drv_twi.h"
#include "tmp116.h" // the main header file
#include "app_error.h"
#include "nrf_delay.h"
...
#define TMP_CONFIGURATION 		0x01
#define TMP_HIGH_LIMIT 0x02
#define TMP_LOW_LIMIT 0x03
#define TMP_EEPROM_Unlock 0x04
#define TMP_EEPROM1 0x05
#define TMP_EEPROM2 0x06
#define TMP_EEPROM3 0x07
#define TMP_EEPROM4 0x08
#define TMP_DEVICEID 0x0F

// I'm probably doing  the instance stuff wrong?
#define TWI_INSTANCE_ID1     0
static const nrf_drv_twi_t m_twi = NRF_DRV_TWI_INSTANCE(TWI_INSTANCE_ID1);
...
static uint8_t 	triggerType = TRIGGER_EXTERNAL_EDGE;		// Trigger Setting
...
void I2C_WriteSingleByte(uint8_t DRV260x_REG, uint8_t DRV260x_DATA)
{
	...
	uint8_t reg[2] = {DRV260x_REG, DRV260x_DATA};
...
uint8_t I2C_ReadSingleByte(uint8_t DRV260x_REG)
{
		ret_code_t err_code;
		uint8_t sample_data;
		err_code = nrf_drv_twi_tx(&m_twi, DRV2605_ADDR, &DRV260x_REG, 1, true);
		if (NRF_SUCCESS == err_code){
				err_code = nrf_drv_twi_rx(&m_twi, DRV2605_ADDR, &sample_data, 1);
		}
		APP_ERROR_CHECK(err_code);
		return sample_data;		
}
...
void Haptics_Init(void)
{
	...
	Haptics_LoadSwitchSelect(ACTUATOR_ERM);     		// Select ERM using load switch
	...
	//I2C_WriteSingleByte(DRV260x_ODT,0x80);
...
void Haptics_RunAutoCal_LRA(void)
{
	...
	I2C_WriteSingleByte(DRV260x_MODE, Auto_Calibration);
	I2C_WriteSingleByte(DRV260x_AUTOCAL_MEMIF, AutoCalTime_1000MS);
	I2C_WriteSingleByte(DRV260x_GO, GO);
...
void Haptics_OutputWaveform(const Waveform waveform)
{
	...
	if(playEffect)
	{
//		//P3OUT &= 0xFB;		// Set Trigger Pin Low
		...
		case MODE_PWM:		// PWM Mode
			...
			{
				//TA1CCR1 = waveform.data[k]+ waveform.data[k]/4 + waveform.data[k]/16;  // Set PWM duty cycle - 1.316x
				for(j=0;j<waveform.data[k+1];++j)	// Count number of 5ms intervals
				{
					nrf_delay_ms(50); //timerdelay(tickdelay);			// Time delay before next change in amplitude
				}
			}
		...
		case MODE_RTP:		// RTP Mode
			...
			for(k=0; k<waveform.length; k=k+2)			// Go through waveform data array of time-value pairs
			{
				...
				for(j=0;j<waveform.data[k+1];++j)		// Count number of 5ms intervals
				{
					nrf_delay_ms(50); //timerdelay(tickdelay);				// Time delay before change in amplitude
				}
			}
...
{
			//need to re-write for nordic
	switch(inputMode)
	{
	case MODE_PWM:		// PWM Mode
		//need to re-write for nordic
//		P3DIR |= BIT2; 			// P3.2 = Output
//		P3SEL |= BIT2;			// P3.2 = PWM Output
		break;
	case MODE_RTP:		// RTP Mode
		//need to re-write for nordic
	//		P3OUT &= 0xFB;			// P3.2 = Low
//		P3SEL &= 0xFB;			// P3.2 = GPIO
//		P3DIR |= 0x04;			// P3.2 = Output
		break;
	case MODE_ANALOG:	// Analog Mode
		//P3DIR &= 0xFB;			// P3.2 = Hi-Z (MSP430 Pin Hi-Z)
		break;
	case MODE_A2H:
		//P3DIR &= 0xFB;			// P3.2 = Hi-Z (MSP430 Pin Hi-Z)
		break;
...
void Haptics_EnableTrigger(void)
{
	...
	{
					//need to re-write for nordic
//		P3OUT |= 0x04;								// Set Trigger Pin High
//		timerdelay(100);
//		P3OUT &= 0xFB;								// Set Trigger Pin Low
	}
	else if(triggerType == TRIGGER_EXTERNAL_LEVEL)
...
//			P3OUT &= 0xFB;								// Set Trigger Pin Low
		...
		{
						//need to re-write for nordic
//			P3OUT &= ~0x04;								// Set Trigger Pin Low
		}
	}
}
...
void Haptics_DisableAmplifier(void)
{
	if(Haptics_IsDisableAllowed())
		nrf_delay_ms(50); //added to compile, need to re-write
		//P3OUT &= 0xF8;             		// Disable Amplifier, Clear Trigger, Clear EN LED
}
